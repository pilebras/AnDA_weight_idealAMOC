...
def AnDA_analog_forecasting(x, AF):
    """ Apply the analog method on catalog of historical data to generate forecasts = xf(t) (from a x state => xa(t-dt)). """
    ...
    while (stop_condition !=1):
        ...
        # in case of global approach => only ones in the neighborhood matrix 
        ...
        kdt = KDTree(AF.catalog.analogs[:,i_var_neighboor], leaf_size=50, metric='euclidean') ## here the distance could be changed 
        ...
        for i_N in range(0,N):
            ...
            # method "locally-linear"
                ...
                # define analogs, successors and weights FOR EACH LLR (ie for each members of the EnKF)
                X = AF.catalog.analogs[np.ix_(index_knn[i_N,:],i_var_neighboor)] # in 'i_var_neighboor' dimension           
                ...
                Cxx2 = np.dot(w**2 * Xr.T,Xr) ### for the covariance
                Cxy  = np.dot(w    * Y.T, Xr)
                ...
                # weighted mean
                xf_mean[i_N,i_var] = np.dot(X0r,beta) ## forecast at time step (k) from xa(k-1)
                pred = np.dot(Xr,beta)
                res = Y-pred
